...
Debug-fast mode (`--debug-fast`) is only for verifying code correctness. Full-run results are in `results/metrics/` and `results/tables/`.
...
Build report assets — copies selected figures into `report/figures/` and exports compact LaTeX table snippets into `report/tables/`:

```bash
python -m src.build_report_assets
python -m src.export_report_tables
```

`build_report_assets` copies figures. `export_report_tables` reads `results/tables/*.csv` and writes the `.tex` snippets. Both must run before compiling the PDF.
