...
def sort_ablation_by_dataset_and_f1(df: pd.DataFrame) -> pd.DataFrame:
    """Sort ablation rows by dataset then f1_macro descending; keep top 4 per dataset to stay within page budget."""
    if "f1_macro" not in df.columns:
        return df
    sorted_df = df.sort_values(["dataset", "f1_macro"], ascending=[True, False])
    return sorted_df.groupby("dataset", as_index=False, group_keys=False).head(4)
...
def export_report_tables() -> list[Path]:
    ...
    generated = [
        ...
        export_table(
            ...
            transform=sort_ablation_by_dataset_and_f1,
        ),
        export_table(
